...
impl Scratchpad {
    ...
    pub(crate) fn write_stl(&self) -> Result<(), opencascade::Error> {
        self.shape.write_stl("./scratchpad.stl")
    }
}
...
pub fn shape() -> Shape {
    ...
    let mid = Workplane::xy()
        .rect(bx, by)
        .fillet((top_fillet - bottom_fillet) / 3.0)
        .transform(
    ...
    let top_wire = Workplane::xy()
        ...
        .arc(
            (curve, curve * tension),
            (0.0, ty / 2.0),
            (curve, ty - curve * tension),
        )
        .arc(
            (curve, ty - curve * tension),
            (tx / 2.0, ty),
            (tx - curve, ty - curve * tension),
        )
        .arc(
            (tx - curve, ty - curve * tension),
            (tx, ty / 2.0),
            (tx - curve, curve * tension),
        )
        .arc(
            (tx - curve, curve * tension),
            (tx / 2.0, 0.0),
    ...
    let keycap: Solid = Solid::loft([&base, &mid, &top_wire]).into();
